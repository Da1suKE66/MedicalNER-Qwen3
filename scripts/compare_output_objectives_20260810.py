...
class StructuredCompletionStoppingCriteria(StoppingCriteria):
    ...
    def __call__(self, input_ids, scores, **kwargs):
        ...
        # row and stop only after all rows have emitted a complete object; an
        # earlier implementation stopped the batch when the first row closed,
        # silently truncating the remaining rows when batch_size > 1.
